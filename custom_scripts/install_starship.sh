...
echo -e "\e[32mCloning dotfiles repo\e[0m"
git clone https://github.com/RKolibri/dotfiles ~/dotfiles

echo -e "\e[32mCreating directory if it does not exist\e[0m"
mkdir -p ~/.config

echo -e "\e[32mCopying files from source to destination\e[0m"
cp ~/dotfiles/config_folder/starship.toml ~/.config/starship.toml

echo -e "\e[32mDisabling password authentication\e[0m"
sudo sed -i 's/#PasswordAuthentication yes/PasswordAuthentication no/g' /etc/ssh/sshd_config


echo -e "\e[32mRemoving dotfiles dir\e[0m"
sudo rm -r ~/dotfiles
